...
# Import the custom transformers from your source code
from src.utils.data_processing import AggregateFeatureCreator, TimeFeatureExtractor

# --- Test Fixture ---
# A fixture is a function that provides a fixed baseline state for tests.
# This fixture creates a small, simple DataFrame that we can use as input for our tests.
@pytest.fixture
def sample_dataframe():
    ...
    data = {
        ...
        'CustomerId': ['A', 'B', 'A', 'C'],
        'TransactionStartTime': ['2025-01-01 10:00:00', '2025-01-02 15:30:00', '2025-01-05 20:00:00', '2025-01-05 20:00:00'],
...
def test_time_feature_extractor(sample_dataframe):
    """
    Tests that the TimeFeatureExtractor correctly creates new time-based columns.
    """
    ...
    expected_new_columns = ['TransactionHour', 'TransactionDay', 'TransactionMonth', 'TransactionYear', 'DaysSinceFirstTransaction']
    ...
    # For the third transaction (index 2), which is customer A's second transaction,
...
def test_aggregate_feature_creator(sample_dataframe):
    """
    Tests that the AggregateFeatureCreator correctly calculates customer-level statistics.
    """
    # Arrange
    ...
    # Assert: Check if the new columns were created
    expected_new_columns = ['TotalTransactionAmount', 'AverageTransactionAmount', 'TransactionCount', 'StdDevTransactionAmount']
    for col in expected_new_columns:
        assert col in transformed_df.columns
