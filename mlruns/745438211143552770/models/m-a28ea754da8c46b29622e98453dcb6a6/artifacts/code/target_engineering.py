...
def create_target_variable(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_with_target = pd.merge(
        df, high_risk_customers, on="CustomerId", how="left"
    )
    ...
    df_with_target["is_high_risk"] = (
        df_with_target["is_high_risk"].fillna(0).astype(int)
    )
...
def calculate_rfm_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rfm = df.groupby("CustomerId").agg(
        {
            "TransactionStartTime": lambda date: (
...
def define_high_risk_label(rfm_df_clustered: pd.DataFrame) -> pd.DataFrame:
    ...
    cluster_summary = (
        rfm_df_clustered.groupby("Cluster")
    ...
    rfm_df_clustered["is_high_risk"] = rfm_df_clustered["Cluster"].apply(
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"Error: The data file was not found. Please ensure '{data_path}' exists."
        )
        exit()
    ...
    print(
        df_with_target[["TransactionId", "CustomerId", "is_high_risk"]].head()
    )
